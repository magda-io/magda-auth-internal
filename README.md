...
| ---------- | -------------- | ----- |
| **`v4.x`** (from `v4.0.0-alpha.0`) | **v7.0.0 or above** | Connects to both its databases (`session-db` and the `auth` DB) over **TLS** when the database enforces SSL. Uses the versioned `magda.db-client-sslmode-env-v1` Helm helper contract plus `magda.db-client-ca-env-v1` for `sslmode: verify-ca`/`verify-full` server-certificate verification (needs `magda-core` `>= 7.0.0-alpha.1`), and runs on **Node.js 22**. |
