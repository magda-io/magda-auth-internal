...
function expectFailureRedirect(response: any, expectedErrorMessage: string) {
    expect(response.header.location).to.include(
        "http://localhost:6100/sign-in-redirect?result=failure"
    );
    const url = new URL(response.header.location);
    expect(url.searchParams.get("errorMessage")).to.include(
        ...
        it("should handle successful login with valid credentials", async () => {
            ...
            const mockPool = {
                query: sandbox.stub().callsFake((sql, params) => {
                    ...
                    if (params && params[0] === testUsername) {
            ...
            expect(response.header.location).to.include(
                "http://localhost:6100/sign-in-redirect?result=success"
            );
        });
    ...
    describe("Error Handling", () => {
        ...
        it("should handle system errors during authentication", async () => {
            ...
            const mockPool = {
                query: sandbox
                    .stub()
                    .rejects(new Error("System error during authentication")),
                end: sandbox.stub().resolves()
            };
